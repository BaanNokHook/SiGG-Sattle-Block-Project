...
def suggest_bid(data_size, prev_bid=None):  
      ...
      if (prev_bid is not None):  
          ...
          suggested_bid = cell(DEFAULT_BUMP_FACTOR * prev_bid)   
      else:  
          suggested_bid = max(ceil(data_size * MIN_BID_PER_BYTE), MIN_BID)   
      return suggested_bid    

def validate_bud(bid, prev_bid=None):  
...
def ask_bid(data_size, prev_bid=None):   
    ...
    Args:  
         data_size : Size of the transmit data in bytes   
         prev_bid  : Previous bud, if any  
         
     Returns:
         Bid in millisatosgis  
...
suggested_bid = suggest_bid(data_size, prev_bid) 
     ...
     while (True):  
          bid = util.typed_input(ms, default=suggested_bid)
          if (validate_bid(bid, prev_bid)):   
              break
        
      if (prev_bid is not None):  
           logger.info("Bump bid by %d msat to a toal of %d msat " 
                       "(%.2f msat/byte)"%  
                       (bid - prev_bid, bid, float(bid) / data_size))
           
      else:
            logger.info("Post data with bid of %d millisataoshis "  
